...
function renderActiveDay() {
  ...
  exercises.forEach(exercise => {
    ...
    setupDragHandle(dragHandle, row, exercise.id);
...
function setupDragHandle(handle, row, exerciseId) {
  ...
  handle.addEventListener("pointerdown", event => {
    ...
    const move = moveEvent => {
      ...
      const siblings = [...routineBody.querySelectorAll("tr:not(.is-dragging)")];
      const insertBeforeRow = siblings.find(candidate => {
        const rect = candidate.getBoundingClientRect();
        return moveEvent.clientY < rect.top + rect.height / 2;
      });

      if (insertBeforeRow) {
        routineBody.insertBefore(row, insertBeforeRow);
      } else {
        routineBody.appendChild(row);
      }
    };
    ...
    const finish = finishEvent => {
      ...
      if (moved) persistCurrentDomOrder();
...
function persistCurrentDomOrder() {
  ...
  const byId = new Map(database.days[activeDay].map(exercise => [exercise.id, exercise]));
  database.days[activeDay] = ids.map(id => byId.get(id)).filter(Boolean);
  saveDatabase();
}
...
function moveExerciseByKeyboard(exerciseId, direction) {
  ...
  requestAnimationFrame(() => {
    routineBody.querySelector(`tr[data-exercise-id="${exerciseId}"] .drag-handle`)?.focus();
  });
}
...
function autoScrollWhileDragging(clientY) {
  ...
  if (clientY > window.innerHeight - threshold) window.scrollBy(0, speed);
}
